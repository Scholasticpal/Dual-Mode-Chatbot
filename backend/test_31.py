...
async def test_model(model_name: str, version: str):
    llm = ChatGoogleGenerativeAI(model=model_name, temperature=0, api_version=version, max_retries=0)
...
async def main():
    await test_model("gemini-3.1-flash-lite", "v1beta")
    await test_model("gemini-3.1-flash-lite", "v1alpha")
